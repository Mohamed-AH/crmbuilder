...
| `REMINDER_HEALTHCHECK_URL` | *(unset)* | Healthchecks.io ping URL for the reminder pass. Pinged when a pass completes — `<url>/fail` when a workspace's delivery failed, so silence ("the deployment is gone") and failure ("it is running and something is broken") stay distinguishable. A bearer credential: environment only, never `platform`, never logged. Unset means the pass simply has no dead-man's switch. **It must not carry a username or password.** `https://user:pw@host/ping/<uuid>` is refused with a warning naming the reason, because `fetch` will not send credentials in a URL — so the check would silently never authenticate. Before §61 it was worse: the refusal came from inside `fetch`, in an error message carrying the whole URL, which was then logged. Put the credential in the path, as Healthchecks itself does |
...
| `RATE_TELEGRAM_MAX` | `12` | Telegram chat lookups per window per IP. Higher than `RATE_HOOK_TEST_MAX` and on its own bucket because the rhythm differs, not the risk: finding a chat is legitimately pressed several times in a row — paste token, press, realise you have not messaged the bot yet, message it, press again — and spending the test-send allowance on that would refuse the send that proves the setup worked |
