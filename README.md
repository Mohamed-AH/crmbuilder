...
- **Chasing what is overdue** — open an overdue record and it writes the reminder: who it is to (following a link to the customer when the invoice itself carries no address), what is owed, how many days late, and the line every real dunning email carries. It hands the draft to **your own email app**, from your own address — no key, no sub-processor, nothing sent from here. The preview is the point: the message is built from the record as it stands, so a stale amount is a demand sent to somebody who has already paid.
- **Sending those reminders yourself, optionally** — connect your own **Resend** or **Postmark** account in Settings and an owner or member can send the chaser in one press instead of opening a draft. Your key, your domain, your billing; the key never leaves the server, is never shown back, and is not in any backup. The draft stays the default and works with no setup at all.
- **Remembering who already chased it** — every reminder sent or drafted is recorded on the record, so the button reads *Chase again* and the preview names who chased it and when. It syncs like the rest of the record and merges by union, so two colleagues chasing on the same afternoon both leave a trace rather than one overwriting the other.
