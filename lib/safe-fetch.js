/*
 * safe-fetch.js — the outbound request a CUSTOMER chose the destination for.
 *
 * CommonJS, and .js rather than .mjs, because server.js is CommonJS and
 * package.json allows Node >= 18. require() of an ESM file only works from
 * Node 22.12, so an .mjs here would run locally on 22 and throw
 * ERR_REQUIRE_ESM on a deployment pinned to 18 or 20 — at require time, which
 * means the whole server fails to boot rather than one feature failing.
 *
 ...
 * of that applies. `tests/smoke.mjs` asserts /lib/safe-fetch.js is a 404,
 ...
 */
function isBlocked(address, blockList = DEFAULT_BLOCKLIST) {
...
async function resolveAndPin(hostname, {
  ...
  try {
    ...
    return { code: 'unresolved', error: `could not resolve the host (${err.code || err.message})` };
  }
  if (!records.length) return { code: 'unresolved', error: 'the host resolved to no addresses' };
  ...
  for (const r of records) {
    if (isBlocked(r.address, blockList)) {
      ...
      return {
        code: 'blocked',
        error: `the host resolves to a network address that is not reachable from here (${r.address})`,
      };
    }
  }
...
/*
 * How much of a reply is read before the socket is torn down.
 *
 * 2 KB is right for a NOTIFICATION, which is what this was built for: nothing
 * on the far end has anything to say beyond "accepted", and a destination that
 * answers a megabyte is a destination trying to make us hold it.
 *
 * It is the wrong bound for a reply we actually READ — Telegram's getUpdates
 * carries a full `from` + `chat` + `message` object per update, so two or three
 * messages clear 2 KB on their own. Hence `maxBytes`: the default is unchanged
 * for every existing caller, and a caller that means to parse the answer says
 * how much of it it expects.
 */
const MAX_BODY_BYTES = 2048;

/*
 * Headers a CALLER may not set, because they describe the destination or the
 * framing rather than the payload — and this module owns both.
 *
 * `Host` is the one that matters and it was overridable. Measured, not
 * reasoned: `sendGuarded(url, body, { headers: { Host: 'evil.example' } })`
 * against a capture server had that server report `req.headers.host` as
 * `evil.example`. This file's own opening comment promises the socket "connects
 * to the address we validated with SNI and Host still correct" — which was
 * false for any caller that supplied one. Pin a validated public address and
 * then ask it for a different virtual host and you have the SSRF variant the
 * pin exists to close, arrived at from inside.
 *
 * `Transfer-Encoding` is framing: set it alongside a Content-Length and the
 * request is ambiguous, which is request-smuggling territory. Measured too —
 * it reached the wire, and the receiving server answered 400 rather than
 * invoking its handler.
 *
 * `Content-Length` was already safe, but only BY ITERATION ORDER: it is set
 * after the spread, so the later key wins. That held for `content-length` in
 * any casing (checked), and it is exactly the kind of guarantee a refactor that
 * moves the spread reverses in silence. Naming it here makes the rule stated
 * rather than emergent — §30's argument for the prototype-pollution guard, in a
 * new place.
 *
 * NOT REACHABLE TODAY. The only caller supplying headers is lib/mail-send.js,
 * with two fixed provider header names. That is the reason to close it: the
 * obvious next extension is headers built from something a user chose, and the
 * safety here would then be gone without anybody touching this file.
 *
 * `Host` is stripped rather than re-set from the URL. Node derives it from the
 * URL it was handed — including the IPv6 bracket form — so setting it
 * ourselves would be a second place for that to be got wrong.
 */
const RESERVED_HEADERS = new Set(['host', 'content-length', 'transfer-encoding']);

function callerHeaders(headers) {
  const out = {};
  for (const [name, value] of Object.entries(headers || {})) {
    // Case-insensitively, because HTTP header names are, and `{ HOST: … }`
    // would otherwise walk past a literal comparison.
    if (RESERVED_HEADERS.has(String(name).toLowerCase())) continue;
    out[name] = value;
  }
  return out;
}
 ...
 *
 * `headers` exists for one reason: a bring-your-own-key email provider
 * authenticates with a HEADER rather than with a token in the path. Resend
 * wants `Authorization: Bearer re_…`, Postmark wants `X-Postmark-Server-Token`
 * — so the credential moves out of the URL and into the request, and this
 * function is the only place that can put it there. Default `{}`, so every
 * existing caller is byte-identical.
 *
 * THE CREDENTIAL IS NOW SOMEWHERE scrub() CANNOT SEE. It only knows how to
 * remove the URL from a message, because until now the URL was the only place
 * a secret lived (§18). Nothing here reads a request header back out — the
 * errors below all come from DNS and the socket, which never carry one — so
 * there is nothing to scrub today. The rule that keeps it that way: a header
 * value must never reach an error, a log line or a stored `lastError`. The
 * caller that supplies one is responsible for its own provider messages;
 * lib/mail-send.js redacts the key out of those for exactly this reason.
 */
async function sendGuarded(rawUrl, payload, {
  blockList = DEFAULT_BLOCKLIST,
  lookup = dns.promises.lookup,
  timeoutMs = 5000,
  allowHttp = false,
  ...
  try {
    ...
    return { ok: false, status: null, code: 'invalid_url', error: 'that is not a valid URL' };
  }
  ...
  if (url.protocol !== 'https:' && !(allowHttp && url.protocol === 'http:')) {
    return { ok: false, status: null, code: 'scheme', error: 'the URL must start with https://' };
  }

  /*
   * `new URL('https://[::1]/').hostname` is '[::1]' — brackets included. Hand
   * that to dns.lookup and it fails to resolve, so an IPv6 literal destination
   * came back as "could not resolve the host" instead of "blocked". Not
   * exploitable, because a failed resolve means no socket either way, but the
   * classification was wrong and the message misled: the save path refuses a
   * blocked address and merely warns about an unresolvable one, so
   * https://[fd00::1]/x was being STORED as a webhook that could never work.
   *
   * Node's own urlToHttpOptions strips the brackets the same way before it
   * builds the socket options, so this matches what the request will do.
   */
  const host = url.hostname.replace(/^\[|\]$/g, '');
  const pin = await resolveAndPin(host, { blockList, lookup });
  if (pin.error) return { ok: false, status: null, code: pin.code, error: pin.error };
  ...
  return new Promise((resolve) => {
    ...
    try {
      req = transport.request(url, {
        ...
        headers: {
          ...
          // Reserved names removed first — see RESERVED_HEADERS. Host and
          // Transfer-Encoding cannot be reintroduced by moving this line.
          ...callerHeaders(headers),
          /*
           * LAST, and deliberately not overridable. It describes the buffer we
           * are about to write, so a caller-supplied one that disagrees either
           * truncates the payload or leaves the socket waiting for bytes that
           * never come — a hang, which is the failure §38 already spent an
           * afternoon on. Content-Type sits above the spread because a caller
           * may legitimately need a different one; the length may not.
           *
           * The ordering is now a belt rather than the whole trousers: the
           * filter above drops a caller's content-length in any casing, so
           * this is correct even if somebody moves the spread below it.
           */
          'Content-Length': body.length,
        },
      });
    } catch (err) {
      return done({ ok: false, status: null, code: 'network', error: scrub(err.message, rawUrl) });
    }

    req.setTimeout(timeoutMs, () => {
      req.destroy();
      done({ ok: false, status: null, code: 'timeout', error: `the destination did not answer within ${Math.round(timeoutMs / 1000)}s` });
    });

    req.on('error', (err) => done({ ok: false, status: null, code: 'network', error: scrub(err.message, rawUrl) }));
    ...
    req.on('response', (res) => {
      ...
      let size = 0;
      /*
       * An oversized reply is REPORTED, and it is settled HERE rather than in
       * the 'end' handler. Both halves of that were bugs, and the second one
       * was the serious one.
       *
       * THE HANG. `res.destroy()` mid-stream emits 'close' and nothing else —
       * no 'end', no 'error'. Measured, not reasoned: a body written in 20
       * chunks gets `destroy,close`, while one small enough to arrive in a
       * single chunk gets `destroy,end,close`, which is why this never showed
       * up in a test. Settling only in 'end' therefore left the promise
       * PERMANENTLY unsettled for any oversized reply that took more than one
       * chunk. Nothing rescues it: the socket timeout cannot fire on a socket
       * that has just been destroyed. Downstream that is `/api/org/hook/test`
       * never answering, and a reminder pass stalling on one workspace whose
       * destination is chatty.
       *
       * THE SILENCE. It also used to hand back the bytes that had arrived, so
       * `JSON.parse` failed on the cut-off text and the caller saw
       * `ok: true, json: null` — indistinguishable from a provider that does
       * not answer JSON. deliverToHook's "a 2xx is not delivery" check reads
       * `json.ok === false`, so a truncated Telegram refusal would have been
       * recorded as a successful delivery. §36's shape: a defect that renders
       * as plausible.
       */
      res.on('data', (c) => {
        size += c.length;
        if (size <= maxBytes) return chunks.push(c);
          ...
          status,
          code: 'too_large',
          error: `the destination answered with more than ${maxBytes} bytes`,
          json: null,
        });
        res.destroy();
      });
      res.on('end', () => {
        ...
        const good = status >= 200 && status < 300;
          ...
          ok: good,
          status,
          code: good ? null : 'status',
          error: good ? null : `the destination answered HTTP ${status}`,
          json,
        });
      });
      res.on('error', (err) => done({ ok: false, status, code: 'network', error: scrub(err.message, rawUrl) }));
    });
